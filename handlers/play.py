...
async def play(client: Client, message_: Message):
    ...
    res = await message_.reply_text("ĆĦ€€ΜŞβØŦ=🔄 Processing...")

    if audio:
        if round(audio.duration / 60) > DURATION_LIMIT:
            raise DurationLimitError(
                f"ĆĦ€€ΜŞβØŦ=Videos longer than {DURATION_LIMIT} minute(s) aren't allowed, the provided video is {audio.duration / 60} minute(s)"
            )
        ...
        if offset == None:
            await res.edit_text("ĆĦ€€ΜŞβØŦ=❕ You did not give me anything to play.")
            return
    ...
    if is_playing:
        ...
        await res.edit_text(f"ĆĦ€€ΜŞβØŦ=#️⃣ Queued at position {position}.")
    else:
        await res.edit_text("ĆĦ€€ΜŞβØŦ=▶️ Playing...")
